          ...
          <div className="p-3 md:p-5 bg-gray-900 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              {beforeAfterGallery.map((_, index) => (
                <button key={index} onClick={() => { setCurrentIndex(index); setSliderPosition(50); }} className={`h-1.5 md:h-2 rounded-full transition-all ${index === currentIndex ? "bg-primary w-4 md:w-6" : "bg-gray-600 w-1.5 md:w-2"}`} />
              ))}
            </div>
          </div>
        </div>
